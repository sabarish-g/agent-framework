...
run_query_agent = AssistantAgent(
    ...
    system_message="You are a helpful AI assistant. Use the query and return the answer using the tool Solve tasks using your tools.",
)
...
async def main():
    ...
    team = RoundRobinGroupChat([generate_query_agent, run_query_agent], max_turns=5)

    # Run the group chat with a question
    stream = team.run_stream(task="What smart phone is running on which software version?")
